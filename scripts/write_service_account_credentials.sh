...
terraform output DPSNonprodServiceAccount_encrypted_aws_secret_access_key | sed 's/"//g' | base64 -d | gpg -dq --pinentry-mode loopback --passphrase ${GPG_KEY_PASSPHRASE} |  opw write one-aws DPSNonprodServiceAccount-aws-secret-access-key -
terraform output DPSNonprodServiceAccount_aws_access_key_id | tr -d \\n | sed 's/"//g' | opw write one-aws DPSNonprodServiceAccount-aws-access-key-id -
